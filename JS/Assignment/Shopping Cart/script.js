...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let countItems = Storage.getproducts().length;
  bag.setAttribute("items", countItems);
});

productcontainer.addEventListener("click", (e) => {
  if (e.target.classList.contains("addtocart")) {
    const productId = e.target.getAttribute("productid");
    productslist.every((elem) => {
      if (elem.id == productId) {
        Storage.addtolocalstorage(elem);
        Ui.displayproducts(elem);
        return false;
      }
      return true;
    });
...
document.querySelector(".pccontainer").addEventListener("click", (e) => {
  if (e.target.classList.contains("delete")) {
    const productId = e.target.getAttribute("productid");
    console.log(productId);
    let productArray = Storage.getproducts();
    console.log(productArray);
    productArray.every((elem) => {
      if (elem.id == productId) {
        e.target.parentElement.remove();
        Storage.removeproduct(elem.id);
        return false;
      }
      return true;
    });
